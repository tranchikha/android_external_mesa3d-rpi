...
vk_physical_device_get_spirv_capabilities(const struct vk_physical_device *pdev)
{
    ...
    */
    ...
    caps.GroupNonUniformPartitionedEXT |= f->shaderSubgroupPartitioned | (p->subgroupSupportedOperations & VK_SUBGROUP_FEATURE_PARTITIONED_BIT_NV);
    ...
    caps.CooperativeMatrixConversionQCOM |= f->cooperativeMatrixConversion;
    ...
    caps.BFloat16TypeKHR |= f->shaderBFloat16Type;
    caps.BFloat16DotProductKHR |= f->shaderBFloat16DotProduct;
    caps.BFloat16CooperativeMatrixKHR |= f->shaderBFloat16CooperativeMatrix;
    ...
    caps.CooperativeVectorNV |= f->cooperativeVector;
    caps.CooperativeVectorTrainingNV |= f->cooperativeVectorTraining;
    ...
    caps.Float8EXT |= f->shaderFloat8;
    caps.Float8CooperativeMatrixEXT |= f->shaderFloat8CooperativeMatrix;
    caps.GraphARM |= f->dataGraph;
    caps.UntypedPointersKHR |= f->shaderUntypedPointers;
    caps.FMAKHR |= f->shaderFmaFloat16 | f->shaderFmaFloat32 | f->shaderFmaFloat64;
    caps.Shader64BitIndexingEXT |= f->shader64BitIndexing;
    caps.LongVectorEXT |= f->longVector;
    caps.DescriptorHeapEXT |= f->descriptorHeap;
    caps.DotProductFloat16AccFloat32VALVE |= f->shaderMixedFloatDotProductFloat16AccFloat32;
    caps.DotProductFloat16AccFloat16VALVE |= f->shaderMixedFloatDotProductFloat16AccFloat16;
    caps.DotProductBFloat16AccVALVE |= f->shaderMixedFloatDotProductBFloat16Acc;
    caps.DotProductFloat8AccFloat32VALVE |= f->shaderMixedFloatDotProductFloat8AccFloat32;
    caps.ConstantDataKHR |= f->shaderConstantData;
    caps.AbortKHR |= f->shaderAbort;
